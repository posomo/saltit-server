...
public interface RestaurantControllerV1Swagger {
	@Operation(summary = "home 화면, 식당 요약 정보 제공용 api", description = "식당의 메인 이미지 주소, 페이지 정보," +
		"식당 요약 리스트를 제공합니다.")
	@ApiResponse(responseCode = "200", description = "정상 응답", content = @Content(schema = @Schema(implementation = RestaurantSummaryResponse.class)))
	@ApiResponse(responseCode = "400", description = "유저 위도 경도 정보, 페이지 번호, 페이지 크기 중 하나라도 null일 때", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
	...
	@ApiResponse(responseCode = "200", description = "정상 응답", content = @Content(schema = @Schema(implementation = RestaurantDetailResponse.class)))
